...
<template>
  <div v-for="(group, groupIndex) of groups" :key="groupIndex">
    <div
      ...
      v-for="(elt, eltIndex) of group"
      :key="`${groupIndex}-${eltIndex}`"
    >
      <p>{{ elt.operation.a }} x {{ elt.operation.b }}</p>
      <p
        v-if="elt.totalAttempts > 0"
        :class="getPercentageColor(elt.goodAnswers, elt.totalAttempts)"
      >
        {{ ((100 * elt.goodAnswers) / elt.totalAttempts).toFixed(0) }}%
      </p>
      <p v-else></p>
